...
import { h, mount, api as rawApi, sse, eur, pct, hhmm, dayhhmm, toast, avatar, createMap, taskPin, workerPin, VEHICLE_IT, setClockOffset, serverNow } from '/shared/common.js';
...
// Ops is protected by ARONICA_OPS_TOKEN on the server. Open /ops/?token=… once;
// the token is kept in this browser and sent with every call.
const OPS_KEY = 'aronica.ops_token';
let opsToken = null;
try {
  const q = new URLSearchParams(location.search).get('token');
  if (q) { localStorage.setItem(OPS_KEY, q); history.replaceState(null, '', location.pathname); }
  opsToken = localStorage.getItem(OPS_KEY);
} catch { /* storage blocked */ }
const api = (path, opts = {}) => rawApi(path, { ...opts, headers: { ...(opts.headers ?? {}), ...(opsToken ? { 'X-Ops-Token': opsToken } : {}) } });
let rankJob = null; // job id picked in the ranking panel (null = latest)
let ranking = null;

async function load() {
  try { state = await api('/api/ops/state'); } catch (e) {
    if (e.status === 401) { mount(main, h('div.empty', {}, h('div', {}, h('b', {}, 'Ops è protetto'), h('div.small.muted', {}, 'Apri questa pagina con ?token=<ARONICA_OPS_TOKEN>.')))); return; }
    throw e;
  }
...
export async function mountOps({ mainEl, logEl, settingsEl: setEl, mapEl }) {
  ...
  sse(`/api/ops/stream${opsToken ? `?ops_token=${encodeURIComponent(opsToken)}` : ''}`, (e) => {
