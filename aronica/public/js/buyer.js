...
export function setAccount(kind) { accountKind = kind; if (!/^#\/job\//.test(useHash ? location.hash : memRoute)) route(); }
...
async function renderHome() {
  ...
  const pin = () => {
    const p = f.custom ?? cfg.places.find((x) => x.name === f.place) ?? (f.compiled?.location);
    if (p) { map.set('task', { lat: p.lat, lng: p.lng, html: taskPin(f.custom ? 'Punto scelto' : (p.name ?? p.address)) }); map.view(p, 14); }
  };
  map.onPick = (p) => { f.custom = p; pin(); if (f.compiled) compile(); };

  const locationInput = () => {
    if (f.custom) return { lat: f.custom.lat, lng: f.custom.lng, address: 'Punto sulla mappa' };
    const p = cfg.places.find((x) => x.name === f.place);
    return p ? { lat: p.lat, lng: p.lng, address: p.name } : undefined;
  };
  ...
  function field(p, c) {
    ...
    const assumed = c.assumed.includes(p.key);
    const set = (val) => { f.overrides[p.key] = val; compile(); };
    let input;
    if (p.type === 'bool') {
      input = h('button.chip', { class: v ? 'on' : '', onclick: () => set(!v) }, v ? icon('check') : null, p.label);
      return input;
    }
    if (p.type === 'enum') input = h('select.input', { onchange: (e) => set(e.target.value) }, p.options.map((o) => h('option', { value: o.v, selected: o.v === v }, o.label)));
    else if (p.type === 'number') input = h('input.input', { type: 'number', value: v ?? '', min: p.min, max: p.max, onchange: (e) => set(e.target.value) });
    else input = h('input.input', { value: v ?? '', onchange: (e) => set(e.target.value) });
    return h('label.field', {}, h('span', {}, p.label, p.unit ? ` (${p.unit})` : '', assumed ? h('span.badge.warn', { style: { marginLeft: '6px' } }, 'ipotizzato') : null), input);
  }

  function windowEditor(c) {
    const s = new Date(c.window.start);
    const e = new Date(c.window.end);
    const pad = (n) => String(n).padStart(2, '0');
    const d = h('input.input', { type: 'date', value: `${s.getFullYear()}-${pad(s.getMonth() + 1)}-${pad(s.getDate())}` });
    const t1 = h('input.input', { type: 'time', value: `${pad(s.getHours())}:${pad(s.getMinutes())}` });
    const t2 = h('input.input', { type: 'time', value: `${pad(e.getHours())}:${pad(e.getMinutes())}` });
    const apply = () => {
      const [y, m, dd] = d.value.split('-').map(Number);
      const [h1, m1] = t1.value.split(':').map(Number);
      const [h2, m2] = t2.value.split(':').map(Number);
      f.window = { start: new Date(y, m - 1, dd, h1, m1).toISOString(), end: new Date(y, m - 1, dd, h2, m2).toISOString() };
      compile();
    };
  ...
  function compiledCard(c) {
    const svc = cfg.services.find((s) => s.code === c.service.code);
    const blocking = c.questions.filter((q) => q.blocking);
    const soft = c.questions.filter((q) => !q.blocking);
    const p = c.price;
    return h('div.card', { style: { boxShadow: 'var(--shadow)', display: 'flex', flexDirection: 'column', gap: '12px' } },
      ...
      h('dl.kv', {},
        ...
        ...p.surcharges.flatMap((s) => [h('dt', {}, `${s.label} (+${s.pct}%)`), h('dd.num', {}, eur(s.cents))]),
  ...
  const mine = loadMine();
  const biz = accountKind === 'business';
  mount(panel,
    h('div.seg', {},
