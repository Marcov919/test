...
let cfg = null;
let me = null;          // /api/worker/me payload
let map = null;
let es = null;
let offerTimer = null;
let shownOffer = null;
...
function connect() {
  es?.close();
  es = sse(`/api/stream?wt=${encodeURIComponent(token)}`, onEvent);
...
async function renderLogin() {
  const people = await api('/api/workers/demo');
  const pick = async (id) => {
    const r = await api('/api/worker/login', { method: 'POST', body: { worker_id: id } });
    token = r.token;
...
async function setOnline(v) {
  try {
    await wapi('/api/worker/online', { method: 'POST', body: { online: v } });
    await refresh();
  } catch (e) { toast(e.message, true); }
}
...
function showProof(job) {
  ...
  const submit = async (btn) => {
    ...
    mount(errors);
    try {
      const r = await wapi(`/api/worker/jobs/${job.id}/proof`, { method: 'POST', body: { photos, answers } });
...
function showDone(payout, jobId) {
  ...
  const panel = h('div.panel', {},
    ...
    h('button.btn.primary.lg.block', {
      ...
      onclick: async () => {
        if (stars) { try { await wapi(`/api/worker/jobs/${jobId}/rate-buyer`, { method: 'POST', body: { stars } }); } catch { /* */ } }
        closeOverlay();
      },
    }, 'Continua'));
...
function toggleRealGps(on) {
  ...
  if (!on) { wapi('/api/worker/location', { method: 'POST', body: { real: false } }); return; }
  if (!navigator.geolocation) return toast('GPS non disponibile', true);
  geoWatch = navigator.geolocation.watchPosition(
    (p) => wapi('/api/worker/location', { method: 'POST', body: { lat: p.coords.latitude, lng: p.coords.longitude } }).catch(() => {}),
