...
function prestoTotal({ workerId = null, accountId = null } = {}) {
  const rows = all(
...
export function routeContract({ account, worker, payoutCents }) {
  ...
  const w = prestoTotal({ workerId: worker.id });
  const b = prestoTotal({ accountId: account.id });
  const pair = all(
