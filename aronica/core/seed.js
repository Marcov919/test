...
import { openDb, closeDb, insert, insertMany, run, tx, setSetting, get } from './db.js';
...
export async function seed({ reset = false } = {}) {
  if (reset) {
    for (const t of ['events', 'offers', 'quotes', 'assignments', 'jobs', 'media', 'ratings', 'workers', 'accounts', 'settings']) await run(`DELETE FROM ${t}`);
  }
  let version = 0;
  try { version = Number((await get("SELECT value FROM settings WHERE key = 'seed_version'"))?.value ?? 0); } catch { /* */ }
  if (!reset && version !== SEED_VERSION && (await get('SELECT COUNT(*) AS c FROM workers')).c > 0) return await seed({ reset: true });
  if ((await get('SELECT COUNT(*) AS c FROM workers')).c > 0) return false;
  const now = Date.now();
  return tx(async () => {
  await insert('accounts', { id: 'acc_demo_agent', name: 'Assistente personale (demo)', kind: 'consumer', api_key: DEMO_API_KEY, org: null, created_at: now });
  await insert('accounts', { id: 'acc_demo_business', name: 'Agente di Aurora Eventi (demo)', kind: 'business', api_key: DEMO_BUSINESS_KEY, org: ORG, created_at: now });
  await insert('accounts', { id: CONSOLE_ACCOUNT, name: 'Privato', kind: 'consumer', api_key: null, org: null, created_at: now });
  await insert('accounts', { id: CONSOLE_BUSINESS_ACCOUNT, name: 'Aurora Eventi', kind: 'business', api_key: null, org: ORG, created_at: now });
  await setSetting('seed_version', SEED_VERSION);
  await setSetting('offer_ttl_s', 20);
  await setSetting('simulate_workers', true);
  await setSetting('sim_speedup', 30);

  const rnd = prng(42);
  for (const w of W) {
    await insert('workers', {
    ...
    await insertMany('ratings', starHistory(w.n, w.avg, rnd).map((s, i) => ({
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const url = process.env.DATABASE_URL || process.env.ARONICA_DB || 'data/aronica.db';
  await openDb(url);
  await seed({ reset: process.argv.includes('--reset') });
  console.log(`Seeded ${(await get('SELECT COUNT(*) AS c FROM workers')).c} partners into ${url.replace(/:[^:@/]+@/, ':***@')}. API keys: ${DEMO_API_KEY} (consumer), ${DEMO_BUSINESS_KEY} (business)`);
  await closeDb();
}
