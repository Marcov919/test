...
export async function ratingStats(workerId) {
  ...
  const total = (await get('SELECT COUNT(*) AS c FROM ratings WHERE worker_id = ?', workerId)).c;
  return { count: n, total_count: total, raw_avg: raw, bayes_avg: bayes, window: RATING_WINDOW };
}

export async function metrics(w) {
...
export async function evaluate(w) {
...
export async function enforce(workerId) {
  ...
  if (Object.keys(patch).length) {
    await update('workers', w.id, patch);
    if (patch.tier && patch.tier !== w.tier) {
      await emit('worker.tier_changed', { worker_id: w.id, data: { from: w.tier, to: ev.tier, reasons: ev.reasons } });
    }
  }
  return ev;
}
