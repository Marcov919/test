...
import { compileTask } from '../core/compiler.js';
import { rankCandidates, publicCandidate, WEIGHTS, WEIGHT_LABELS_IT } from '../core/matching.js';
import { createJob, serializeJob, loadJob, listJobsForAccount, rateWorker, summarizeExcluded } from '../core/jobs.js';
import { negotiateRound, acceptQuote, autoNegotiate } from '../core/negotiation.js';
import { buyerCancel } from '../core/dispatch.js';
import { eventsForJob, waitForJobEvent } from '../core/events.js';
import { RATING_TAGS } from '../core/reliability.js';
import { CAPS, ROUTES } from '../core/compliance.js';
import { CITY, GAZETTEER } from '../core/geo.js';
import { HttpError, NO_SUPPLY_IT } from '../core/util.js';
...
export async function searchSupply(args = {}) {
  ...
  const { eligible, excluded } = await rankCandidates(probe);
  return {
...
const handlers = {
  ...
  async compile_task(account, args) { return compileTask(args); },
  async search_supply(account, args) { return await searchSupply(args); },
  async create_job(account, args) { return await createJob(account, args); },
  async negotiate(account, args) {
    await ownJob(account, args.job_id);
    return await negotiateRound(args.job_id, { windows: args.window ? [args.window] : null, actor: account.name, message: args.message });
  },
  async auto_negotiate(account, args) {
    await ownJob(account, args.job_id);
    const r = await autoNegotiate(args.job_id);
    const job = await serializeJob(await loadJob(args.job_id), { buyer: true });
  ...
  async accept_quote(account, args) {
    await ownJob(account, args.job_id);
    const r = await acceptQuote(args.job_id, args.quote_id, { actor: account.name });
    const job = await serializeJob(await loadJob(args.job_id), { buyer: true });
    return {
  ...
  async get_job(account, args) { return await serializeJob(await ownJob(account, args.job_id), { buyer: true, events: !!args.include_events }); },
  async list_jobs(account, args) { return { jobs: await listJobsForAccount(account.id, Math.min(100, Number(args.limit ?? 20))) }; },
  async wait_for_update(account, args) {
    ...
    let events = await eventsForJob(args.job_id, since);
    if (!events.length && timeout) { await waitForJobEvent(args.job_id, timeout, { sinceSeq: since }); events = await eventsForJob(args.job_id, since); }
    return { status: (await get('SELECT status FROM jobs WHERE id = ?', args.job_id)).status, events, next_since_seq: events.length ? events[events.length - 1].seq : since };
  },
  async cancel_job(account, args) { await ownJob(account, args.job_id); return await buyerCancel(args.job_id, args.reason); },
  async rate_worker(account, args) { await ownJob(account, args.job_id); return await rateWorker(args.job_id, args); },
};
...
export async function accountFromKey(key) {
  if (!key) return null;
  return await get("SELECT * FROM accounts WHERE api_key = ? AND kind IN ('consumer','business')", key) ?? null;
}
